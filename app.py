...
def result():
   ...
   # else:
    ...
    if(sex==1):
        prediction=np.array([[fever,cold,rigor,fatigue,headace,bitter_tongue,vomitting,diarrhea,Convulsion,Anemia,jundice,cocacola_urine,hypoglycemia,prostraction,hyperpyrexia,0,1]])
        preds=model.predict(prediction)
        if preds==1:
            pd="You have Malaria"
        else:
            pd="You Don't have Malaria"
    else:
        prediction=np.array([[fever,cold,rigor,fatigue,headace,bitter_tongue,vomitting,diarrhea,Convulsion,Anemia,jundice,cocacola_urine,hypoglycemia,prostraction,hyperpyrexia,1,0]])
        preds=model.predict(prediction)
        if preds==1:
            pd="You have Malaria"
        else:
